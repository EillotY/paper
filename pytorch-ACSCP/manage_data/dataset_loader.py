...
class UCF_CC_50(object):
    ...
    def _create_train_test(self, force_augmentation, kwargs):
        slide_window_params = {'displace': kwargs['displace'], 'size_x': kwargs['size_x'], 'size_y': kwargs['size_y'],
                               'people_thr': kwargs['people_thr']}
        noise_params = {'augment_noise': kwargs['augment_noise']}
        light_params = {'augment_light': kwargs['augment_light'], 'bright': kwargs['bright'],
        ...
        for fold in range(5):
            ...
            train_test = train_test_unit(aug_train_dir_img, aug_train_dir_den, fold_test_dir_img, fold_test_dir_den,
                                         kwargs.copy())
            ...
            if augment_data:
                ...
                augment(train_img_paths, train_lab_paths, train_den_paths, aug_train_dir_img, aug_train_dir_lab,
                        aug_train_dir_den, slide_window_params, noise_params, light_params)
...
class ShanghaiTech(object):
    ...
    def _create_original_density_maps(self, force_create_den_maps):
        ...
        if not all_density_dirs_exist:
            ...
            return
        create_density_map(self.ori_dir_partA_train_img, self.ori_dir_partA_train_lab, self.ori_dir_partA_train_den,
                           mode=self.metadata['gt_mode'])
        create_density_map(self.ori_dir_partA_test_img, self.ori_dir_partA_test_lab, self.ori_dir_partA_test_den,
                           mode=self.metadata['gt_mode'])
        create_density_map(self.ori_dir_partB_train_img, self.ori_dir_partB_train_lab, self.ori_dir_partB_train_den,
                           mode=self.metadata['gt_mode'])
        create_density_map(self.ori_dir_partB_test_img, self.ori_dir_partB_test_lab, self.ori_dir_partB_test_den,
                           mode=self.metadata['gt_mode'])
    ...
    def _create_train_test(self, force_augmentation, kwargs):
        ...
        part_A_train_test = train_test_unit(aug_dir_partA_img, aug_dir_partA_den, self.ori_dir_partA_test_img,
                                            self.ori_dir_partA_test_den, kwargs.copy())
        self.train_test_set.append(part_A_train_test)

        if augment_data_A:
            ori_img_paths = [osp.join(self.ori_dir_partA_train_img, file_name) for file_name in
                             sorted(os.listdir(self.ori_dir_partA_train_img))]
            ori_lab_paths = [osp.join(self.ori_dir_partA_train_lab, file_name) for file_name in
                             sorted(os.listdir(self.ori_dir_partA_train_lab))]
            ori_den_paths = [osp.join(self.ori_dir_partA_train_den, file_name) for file_name in
                             sorted(os.listdir(self.ori_dir_partA_train_den))]
            augment(ori_img_paths, ori_lab_paths, ori_den_paths, aug_dir_partA_img, aug_dir_partA_lab,
                    aug_dir_partA_den, slide_window_params, noise_params, light_params)
        ...
        part_B_train_test = train_test_unit(aug_dir_partB_img, aug_dir_partB_den, self.ori_dir_partB_test_img,
                                            self.ori_dir_partB_test_den, kwargs.copy())
        self.train_test_set.append(part_B_train_test)

        if augment_data_B:
            ori_img_paths = [osp.join(self.ori_dir_partB_train_img, file_name) for file_name in
                             sorted(os.listdir(self.ori_dir_partB_train_img))]
            ori_lab_paths = [osp.join(self.ori_dir_partB_train_lab, file_name) for file_name in
                             sorted(os.listdir(self.ori_dir_partB_train_lab))]
            ori_den_paths = [osp.join(self.ori_dir_partB_train_den, file_name) for file_name in
                             sorted(os.listdir(self.ori_dir_partB_train_den))]
            augment(ori_img_paths, ori_lab_paths, ori_den_paths, aug_dir_partB_img, aug_dir_partB_lab,
                    aug_dir_partB_den, slide_window_params, noise_params, light_params)
