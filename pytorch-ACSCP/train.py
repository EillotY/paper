...
parser.add_argument('--force-den-maps', action='store_true',
                    help="force generation of dentisity maps for original dataset, by default it is generated only once")
parser.add_argument('--force-augment', action='store_true',
                    help="force generation of augmented data, by default it is generated only once")
parser.add_argument('--displace', default=70, type=int,
                    help="displacement for sliding window in data augmentation, default 70")
parser.add_argument('--size-x', default=256, type=int, help="width of sliding window in data augmentation, default 200")
parser.add_argument('--size-y', default=256, type=int,
                    help="height of sliding window in data augmentation, default 300")
parser.add_argument('--people-thr', default=0, type=int,
                    help="threshold of people sliding window in data augmentation, default 200")
parser.add_argument('--not-augment-noise', action='store_true',
                    help="not use noise for data augmetnation, default True")
parser.add_argument('--not-augment-light', action='store_true',
                    help="not use bright & contrast for data augmetnation, default True")
parser.add_argument('--bright', default=10, type=int, help="bright value for bright & contrast augmentation, defaul 10")
parser.add_argument('--contrast', default=10, type=int,
                    help="contrast value for bright & contrast augmentation, defaul 10")
...
parser.add_argument('--train-batch', default=10, type=int,
                    help="train batch size (default 32)")
# Miscs
parser.add_argument('--den-factor', type=float, default=1e3,
                    help="factor to multiply for density maps to avoid too small values")
...
parser.add_argument('--resume', type=str, default='', metavar='PATH',
                    help="root directory where part/fold of previous train are saved")
...
parser.add_argument('--units', type=str, default='',
                    help="folds/parts units to be trained, be default all folds/parts are trained")
parser.add_argument('--augment-only', action='store_true', help="run only data augmentation, default False")
parser.add_argument('--evaluate-only', action='store_true',
                    help="run only data validation, --resume arg is needed, default False")
parser.add_argument('--save-plots', action='store_true',
                    help="save plots of density map estimation (done only in test step), default False")
...
def train(train_test_unit, out_dir_root):
    ...
    optimizer_d_large = torch.optim.Adam(filter(lambda p: p.requires_grad, net.d_large.parameters()), lr=lr,
                                         betas=(args.beta1, args.beta2))
    optimizer_d_small = torch.optim.Adam(filter(lambda p: p.requires_grad, net.d_small.parameters()), lr=lr,
                                         betas=(args.beta1, args.beta2))
    optimizer_g_large = torch.optim.Adam(filter(lambda p: p.requires_grad, net.g_large.parameters()), lr=lr,
                                         betas=(args.beta1, args.beta2))
    optimizer_g_small = torch.optim.Adam(filter(lambda p: p.requires_grad, net.g_small.parameters()), lr=lr,
                                         betas=(args.beta1, args.beta2))
    ...
    data_loader = ImageDataLoader(train_path, train_gt_path, shuffle=True, batch_size=args.train_batch,
                                  test_loader=False)
    data_loader_val = ImageDataLoader(val_path, val_gt_path, shuffle=False, batch_size=1, test_loader=True,
                                      img_width=args.size_x, img_height=args.size_y, test_overlap=overlap_test)
    ...
    for epoch in range(start_step, end_step + 1):
        ...
        for blob in data_loader:
            ...
            if step % disp_interval == 0:
                ...
                print(
                    "epoch: {0}, step {1}/{5}, Time: {2:.4f}s, gt_cnt: {3:.4f}, et_cnt: {4:.4f}, mean_diff: {6:.4f}".format(
                        epoch, step, 1. / fps, gt_count[0], et_count[0], data_loader.num_samples,
        ...
        print(
            "Epoch: {0}, MAE: {1:.4f}, MSE: {2:.4f}, loss gen small: {3:.4f}, loss gen large: {4:.4f}, loss dis small: {5:.4f}, loss dis large: {6:.4f}, loss: {7:.4f}".format(
                epoch, mae, mse, train_loss_gen_small, train_loss_gen_large, train_loss_dis_small, train_loss_dis_large,
...
def test(train_test_unit, out_dir_root):
    ...
    data_loader = ImageDataLoader(val_path, val_gt_path, shuffle=False, batch_size=1, test_loader=True,
                                  img_width=args.size_x, img_height=args.size_y, test_overlap=overlap_test)
    mae, mse = evaluate_model(pretrained_model, data_loader, save_test_results=args.save_plots,
                              plot_save_dir=osp.join(output_dir, 'plot-results-test/'), den_factor=args.den_factor)
...
def main():
    ...
    dataset = dataset_loader.init_dataset(name=args.dataset, force_create_den_maps=force_create_den_maps,
